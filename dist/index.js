...
import { CodeGraphProgressComponent, formatProgress } from "./progress-component.js";
const CODEGRAPH_WORKER_COMMANDS = ["init", "sync", "status", "uninit"];
const CODEGRAPH_LOCAL_COMMANDS = ["version", "update"];
const CODEGRAPH_COMMANDS = [...CODEGRAPH_WORKER_COMMANDS, ...CODEGRAPH_LOCAL_COMMANDS, "help"];
const COMMAND_TIMEOUT_MS = 10 * 60 * 1000;
const WORKER_PROGRESS_INTERVAL_MS = 500;
const CUSTOM_UI_COMMANDS = new Set(["init", "sync"]);
/** 把 MCP JSON Schema 转为 pi 可接受的 TypeBox schema。 */
function toTypeBoxSchema(tool) {
    return Type.Unsafe(tool.inputSchema);
}
/** 按空白切分命令参数，保留简单引号内容。 */
...
function spawnCodegraphWorker(ctx, command, args) {
...
function watchWorkerProcess(proc, command, options = {}) {
    const messages = [];
    let resultMessage;
    let errorMessage;
    let pendingLine = "";
...
async function runCodegraphWorker(ctx, command, args) {
    ...
    try {
        return await watchWorkerProcess(proc, command, {
            onProgressText: (text) => {
                ...
                const now = Date.now();
                if (now - lastUpdateAt < WORKER_PROGRESS_INTERVAL_MS)
                    return;
                lastUpdateAt = now;
...
async function runCodegraphWorkerWithCustomUi(ctx, command, args) {
    if (!ctx.hasUI)
        return runCodegraphWorker(ctx, command, args);
    return ctx.ui.custom((tui, theme, _keybindings, done) => {
        ...
        void watchWorkerProcess(proc, command, {
            onMessage: (message) => {
                ...
                const now = Date.now();
                if (now - lastRenderAt < WORKER_PROGRESS_INTERVAL_MS)
                    return;
                lastRenderAt = now;
                component.update(message);
            },
...
async function runLocalCodegraphUpdate(ctx) {
    const runtime = loadCodeGraphRuntime();
...
function getToolHandler(state, ctx) {
    const runtime = loadCodeGraphRuntime();
...
/** 后台补齐关闭期间的代码变动。 */
async function catchUpSync(state, ctx) {
    const root = findProjectRoot(ctx.cwd);
    if (!root)
        return;
    const runtime = loadCodeGraphRuntime();
...
async function startWatcher(state, ctx) {
    const root = findProjectRoot(ctx.cwd);
    if (!root || state.watcherProject === root)
        return;
    try {
        state.watcher?.close();
    }
...
/** 格式化 status JSON 输出。 */
function formatStatusSummary(json) {
...
/** 格式化 uninit 结果。 */
function formatUninitSummary(result) {
    const data = (result.result?.data ?? {});
    if (data.alreadyRemoved)
        return "CodeGraph 未初始化，无需删除。";
...
async function readInstalledCodegraphVersion() {
    const runtime = loadCodeGraphRuntime();
...
/** 格式化本地更新结果。 */
async function formatUpdateSummary(previousVersion, result) {
...
/** 处理本地维护命令，不读取或修改项目索引。 */
async function handleLocalCommand(command, ctx, state) {
    if (command === "version") {
...
/** 根据 worker 结果生成清晰摘要。 */
function formatWorkerSummary(command, result) {
    if (result.error) {
...
/** 处理 /codegraph 子命令。 */
async function handleCodegraphCommand(argsText, ctx, state) {
    ...
    if (!CODEGRAPH_COMMANDS.includes(subcommand)) {
        ctx.ui.notify(`未知子命令：${subcommand}\n用法：/codegraph init|sync|status|uninit|version|update`, "warning");
        return;
    }
    if (CODEGRAPH_LOCAL_COMMANDS.includes(subcommand)) {
        await handleLocalCommand(subcommand, ctx, state);
        return;
    }
...
function buildPrompt(systemPrompt, selectedTools, cwd) {
    const runtime = loadCodeGraphRuntime();
...
export default function codegraphPiExtension(pi) {
    const state = {};
    const runtime = loadCodeGraphRuntime();
    for (const tool of runtime.toolsModule.tools) {
        pi.registerTool({
            ...
            async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
                ...
                const result = await handler.execute(tool.name, params);
                return {
