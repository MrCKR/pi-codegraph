...
type WorkerCommand = "init" | "index" | "sync" | "status" | "uninit";
...
/** 失败时输出结构化错误并设置退出码。 */
function fail(command: WorkerCommand, error: unknown): never {
...
/** 计算文件变动总数。 */
function changedCount(result: { filesAdded: number; filesModified: number; filesRemoved: number }): number {
	return result.filesAdded + result.filesModified + result.filesRemoved;
}

/** 读取当前索引状态。 */
function readStatus(projectPath: string): Record<string, unknown> {
	const runtime = loadCodeGraphRuntime();
	if (!runtime.codegraphClass) throw new Error("CodeGraph runtime is not loaded");
...
/** 初始化当前项目。 */
async function runInit(projectPath: string, index: boolean): Promise<void> {
	const runtime = loadCodeGraphRuntime();
...
/** 全量建立索引。 */
async function runIndex(projectPath: string, force: boolean): Promise<void> {
	const runtime = loadCodeGraphRuntime();
	const root = runtime.codegraph.findNearestCodeGraphRoot(projectPath);
	if (!root) throw new Error("CodeGraph is not initialized. Run /codegraph init first.");
...
/** 增量同步代码变动。 */
async function runSync(projectPath: string): Promise<void> {
	const runtime = loadCodeGraphRuntime();
	const root = runtime.codegraph.findNearestCodeGraphRoot(projectPath);
	if (!root) throw new Error("CodeGraph is not initialized. Run /codegraph init first.");
...
/** 删除 .codegraph 目录。 */
function runUninit(projectPath: string): void {
	const runtime = loadCodeGraphRuntime();
	const root = runtime.codegraph.findNearestCodeGraphRoot(projectPath);
...
async function main(): Promise<void> {
	...
	const command = commandArg as WorkerCommand | undefined;
...
main().catch((error) => fail((process.argv[2] as WorkerCommand) ?? "status", error));
